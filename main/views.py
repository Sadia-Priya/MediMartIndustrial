...
def doctor_elab_list(request):
    ...
    # Only show tests assigned to this doctor
    elab_tests = eLabSchedule.objects.filter(assigned_doctor=request.user.doctor_profile)

    if request.method == 'POST' and 'verify' in request.POST:
        test_id = request.POST.get('test_id')
        test = get_object_or_404(eLabSchedule, id=test_id, assigned_doctor=request.user.doctor_profile)

        if test.report_file:
            test.report_verified = True
            test.save()
            messages.success(request, f"'{test.test_name}' report verified!")
        else:
            messages.error(request, 'Cannot verify: no report uploaded.')

        return redirect('doctor_elab_list')

    return render(request, 'main/doctor_elab_list.html', {'elab_tests': elab_tests})
